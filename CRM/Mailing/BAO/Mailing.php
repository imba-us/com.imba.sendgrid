...
/**
 * IMBA NOTES:
 * This file has been over-ridden in order to implement changes to the mailing report.
 *
 * Only the report function is altered .
 *
 * Long term maintaining this over-ride is problematic and I believe we should look to find a way not to.
 * The function is called from two places - CRM_Activity_Form_ActivityView &  CRM_Mailing_Page_Report.
 *
 * I think we would be able to implement a pageRun hook that re-directs (possibly conditionally) to a custom page.
 * There is a precedent for this in CRM_Mailing_Page_Report
 *
 *
 *   public function run() {
...
";
  ...
  public static function &report($id, $skipDetails = FALSE, $isSMS = FALSE) {
    ...
    $t = array(
      ...
      'spam' => CRM_Mailing_Event_BAO_SpamReport::getTableName(),
    );
    ...
    $mailing->query("
            SELECT          {$t['job']}.*,
                            ...
                            COUNT(DISTINCT {$t['spam']}.id) as spam
            FROM            {$t['job']}
            LEFT JOIN       {$t['queue']}
            ...
            LEFT JOIN       {$t['spam']}
                    ON      {$t['spam']}.event_queue_id = {$t['queue']}.id
    ...
    while ($mailing->fetch()) {
      ...
      if ($mailing->queue) {
        ...
        $row['spam_rate'] = (100.0 * $row['spam']) / $mailing->queue;
      }
      else {
      ...
      $row['links'] = array(
        ...
        'spam' => CRM_Utils_System::url(
          'civicrm/mailing/report/event',
          "reset=1&event=spam&mid=$mailing_id&jid={$mailing->id}"
        ),
      );
    ...
    if (!empty($report['event_totals']['queue'])) {
      ...
      $report['event_totals']['spam_rate'] = (100.0 * $report['event_totals']['spam']) / $report['event_totals']['queue'];
    }
    else {
      ...
      $report['event_totals']['spam_rate'] = 0;
    }
    ...
    $report['event_totals']['links'] = array(
      ...
      'spam' => CRM_Utils_System::url(
        'civicrm/mailing/report/event',
        "reset=1&event=spam&mid=$mailing_id"
      ),
    );
    ...
    foreach (array(
      ...
      switch ($key) {
        ...
        case 'spam':
          $reportFilter .= "&is_spam_value=1";
          $searchFilter .= "&mailing_spam_status=Y";
          break;
      }
