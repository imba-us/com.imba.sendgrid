...
class CRM_Mailing_Selector_Event extends CRM_Core_Selector_Base implements CRM_Core_Selector_API {
  ...
  function &getColumnHeaders($action = NULL, $output = NULL) {
    ...
    if (!isset($this->_columnHeaders)) {
      ...
      switch ($this->_event_type) {
        ...
        case 'spam':
          $dateSort = CRM_Mailing_Event_BAO_SpamReport::getTableName() . '.time_stamp';
          $this->_columnHeaders = array_merge($this->_columnHeaders, array(
            array(
                'name' => ts('Spam Report'),
              ),
  ...
  function getTotalCount($action) {
    switch ($this->_event_type) {
      ...
      case 'spam':
        $event = new CRM_Mailing_Event_BAO_SpamReport();
        return $event->getTotalCount($this->_mailing_id,
          $this->_job_id,
          $this->_is_distinct,
          FALSE
  ...
  function &getRows($action, $offset, $rowCount, $sort, $output = NULL) {
    switch ($this->_event_type) {
      ...
      case 'spam':
        return CRM_Mailing_Event_BAO_SpamReport::getRows($this->_mailing_id,
          $this->_job_id, $this->_is_distinct,
          $offset, $rowCount, $sort
        );
      break;
    ...
    if (empty($events)) {
      $events = array(
        ...
        'opened' => $this->_is_distinct
         ...
         : ts('Tracked Opens'),
        'spam' => $this->_is_distinct
         ? ts('Unique Spam Reports')
         : ts('Spam Reports'),
