...
class CRM_Report_Form_Mailing_Detail extends CRM_Report_Form {
  ...
  function __construct() {
    ...
    $this->_columns['civicrm_mailing_event_spam_report'] = array(
    	'dao' => 'CRM_Mailing_Event_DAO_SpamReport',
    	'fields' => array(
    		'spam_id' => array(
    			'name' => 'id',
    			'title' => ts('Spam Report'),
				'default' => TRUE,
    		),
    	),
		'filters' => array(
			'is_spam' => array(
				'name' => 'id',
				'title' => ts('Reported As Spam'),
				'type' => CRM_Utils_Type::T_INT,
				'operatorType' => CRM_Report_Form::OP_SELECT,
				'options' => array('' => ts('Any'), '0' => ts('No'), '1' => ts('Yes')),
				'clause' => 'mailing_event_spam_report_civireport.id IS NULL',
			),
		),
		'grouping' => 'mailing-fields',
    );
        ...
        foreach ($table['fields'] as $fieldName => $field) {
          if (CRM_Utils_Array::value('required', $field) ||
            ...
            if (in_array($fieldName, array(
              'unsubscribe_id', 'optout_id', 'forward_id', 'reply_id', 'spam_id'))) {
  ...
  function from() {
    ...
    if (array_key_exists('spam_id', $this->_params['fields']) ||
      is_numeric(CRM_Utils_Array::value('is_spam_value', $this->_params))
    ) {
      if (CRM_Utils_Array::value('is_spam_value', $this->_params) == 1) {
        $joinType = 'INNER';
        $this->_columns['civicrm_mailing_event_spam_report']['filters']['is_spam']['clause'] = '(1)';
      }
      else {
        $joinType = 'LEFT';
      }
      $this->_from .= "
                {$joinType} JOIN  civicrm_mailing_event_spam_report {$this->_aliases['civicrm_mailing_event_spam_report']}
                    ON  {$this->_aliases['civicrm_mailing_event_spam_report']}.event_queue_id = civicrm_mailing_event_queue.id";
    }
    else {
      unset($this->_columns['civicrm_mailing_event_spam_report']['filters']['is_spam']);
    }
