...
class CRM_Report_Form_Mailing_Summary extends CRM_Report_Form {
  ...
  function __construct() {
    ...
    $this->_columns['civicrm_mailing_event_spam_report'] = array(
      ...
      'fields' => array(
        'report_count' => array(
          'name' => 'id',
          'title' => ts('Spam Reported'),
        ),
        'report_rate' => array(
          'title' => 'Confirmed Spam Report Rate',
          'statistics' => array(
            'calc' => 'PERCENTAGE',
            'top' => 'civicrm_mailing_event_spam_report.report_count',
            'base' => 'civicrm_mailing_event_delivered.delivered_count',
          ),
        ),
      ),
  ...
  function select() {
    ...
    $count_tables = array(
      ...
      'civicrm_mailing_event_spam_report',
    );
  ...
  function from() {
    ...
    FROM civicrm_mailing {$this->_aliases['civicrm_mailing']}
      ...
      LEFT JOIN civicrm_mailing_event_spam_report {$this->_aliases['civicrm_mailing_event_spam_report']}
        ON {$this->_aliases['civicrm_mailing_event_spam_report']}.event_queue_id = {$this->_aliases['civicrm_mailing_event_queue']}.id";
  ...
  function formRule($fields, $files, $self) {
    ...
    foreach ($fields['fields'] as $fld => $isActive) {
      if (in_array($fld, array(
        'delivered_count', 'bounce_count', 'open_count', 'click_count', 'unsubscribe_count', 'report_count', 'accepted_rate', 'bounce_rate', 'open_rate', 'CTR', 'CTO', 'report_rate'))) {
        $isError = FALSE;
      }
    }
